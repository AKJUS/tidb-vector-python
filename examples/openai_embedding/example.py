...
embeddings = [
    r.embedding
    for r in client.embeddings.create(input=documents, model=embedding_model).data
]
data_source = [
    {"text": doc, "embedding": emb} for doc, emb in zip(documents, embeddings)
]
...
question_embedding = (
    client.embeddings.create(input=question, model=embedding_model).data[0].embedding
)
related_docs = (
    DocModel.select(
        DocModel.text,
        DocModel.embedding.cosine_distance(question_embedding).alias("distance"),
    )
