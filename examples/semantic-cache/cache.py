...
def generate_embeddings(jinaai_api_key: str, text: str):
    ...
    response = requests.post(
        JINAAI_API_URL, headers=JINAAI_HEADERS, json=JINAAI_REQUEST_DATA
    )
...
def get(
    ...
    with Session(engine) as session:
        result = session.exec(
            select(Cache, Cache.key_vec.cosine_distance(key_vec).label("distance"))
            .order_by("distance")
            .limit(1)
        ).first()
        ...
        return {"key": cache.key, "value": cache.value, "distance": distance}
